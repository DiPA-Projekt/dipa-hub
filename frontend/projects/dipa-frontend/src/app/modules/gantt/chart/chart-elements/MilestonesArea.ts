...
export class MilestonesArea implements IChartElement {
  ...
  setData(data): void {
    ...
    const dataGroup = this.svg.select('g.data-group' + this.milestoneAreaId);
  ...
  draw(offset): void {
    ...
    // dataGroup.attr('transform', 'translate(' + offset.left + ',' + offset.top  + ')');
  ...
  getAreaHeight(): number {
    ...
    // const dataGroup = this.svg.select('g.data-group' + this.milestoneAreaId).node();
// 
    // return dataGroup.getBoundingClientRect().height;
  }
