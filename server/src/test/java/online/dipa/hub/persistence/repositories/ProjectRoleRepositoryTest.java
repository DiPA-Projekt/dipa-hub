...
class ProjectRoleRepositoryTest {
    ...
    class FindByTemplateAndAbbreviation {
        ...
        void should_return_when_project_role_template_template_exists() {
            // GIVEN
            final ProjectRoleTemplateEntity projectRoleTemplate = projectRoleTemplateRepository.findById(1L)
                                                                                               .get();
            final String abbreviation = "PE";

            // WHEN
            final Optional<ProjectRoleEntity> projectTaskTemplate = projectRoleRepository.findByTemplateAndAbbreviation(
                    projectRoleTemplate, abbreviation);

            // THEN
            then(projectTaskTemplate).isNotEmpty()
                                     .get()
