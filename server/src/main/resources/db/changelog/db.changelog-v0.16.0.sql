...
-- changeset id:create-table-event-template
CREATE TABLE IF NOT EXISTS public.event_template
(
    id BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 9223372036854775807 CACHE 1 ),
    title VARCHAR(255),
    ...
    CONSTRAINT "eventTemplateProjectFK" FOREIGN KEY (project_id)
        REFERENCES public.project (id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    CONSTRAINT "eventTemplateProjectTaskResultFK" FOREIGN KEY (result_id)
        REFERENCES public.project_task_result (id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    CONSTRAINT "eventTemplateRecurringTypeFK" FOREIGN KEY (recurring_event_type_id)
        REFERENCES public.recurring_event_type (id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE
)
...
(
    ...
    CONSTRAINT "eventEventTemplateFK" FOREIGN KEY (event_template_id)
        REFERENCES public.event_template (id) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE CASCADE
)