...
--changeset id:insert-recurring-type-tracking-table-template context:weit
...
--changeset id:insert-recurring-type-performance-record-release-template context:weit
INSERT INTO public.recurring_event_type(mandatory, master, title, description, project_property_question_id)
...
--changeset id:migration-recurring-type-project-no-property-question context:weit
INSERT INTO public.recurring_event_type(title, description, mandatory, project_id, master)
SELECT recurring_event_type.title, recurring_event_type.description, mandatory, project.id, false
FROM recurring_event_type, project
WHERE recurring_event_type.project_property_question_id IS NULL

--changeset id:migration-recurring-type-project-with-property-question context:weit
...
--changeset id:fix-documentationLink-label-null context:weit
UPDATE public.project_task_form_field
SET label = 'Link'
WHERE key = 'documentationLink' and label = ''

--changeset id:delete-form-field-termin-project-task-appt-series context:weit
DELETE FROM project_task_form_field
WHERE label = 'Termin'

--changeset id:insert-form-field-start-time-project-task-appt-series context:weit
...
--changeset id:insert-form-field-duration-project-task-appt-series context:weit
INSERT INTO project_task_form_field (value, key, label, required, sort_order, CONTROL_TYPE, type, show, result_id)
SELECT '', 'duration', 'Dauer', true, 3, 'TEXTBOX', 'NUMBER', true, id
FROM project_task_result
WHERE result_type = 'TYPE_APPT_SERIES'

--changeset id:insert-form-field-rule-pattern-project-task-appt-series context:weit
INSERT INTO project_task_form_field (value, key, label, required, sort_order, CONTROL_TYPE, type, show, result_id)
SELECT 'FREQ=MONTHLY;BYMONTHDAY=10;INTERVAL=1', 'appointment', 'Terminregel', true, 4, 'RRULE', 'TEXT', true, id
FROM project_task_result
WHERE result_type = 'TYPE_APPT_SERIES'

--changeset id:insert-form-field-start-date-project-task-appt-series context:weit
INSERT INTO project_task_form_field (value, key, label, required, sort_order, CONTROL_TYPE, type, show, result_id)
SELECT '', 'startDate', 'Start', true, 5, 'TEXTBOX', 'DATE', true, id
FROM project_task_result
WHERE result_type = 'TYPE_APPT_SERIES'

--changeset id:insert-form-field-end-date-project-task-appt-series context:weit
INSERT INTO project_task_form_field (value, key, label, required, sort_order, CONTROL_TYPE, type, show, result_id)
...
--changeset id:update-form-field-serie-title-required context:weit
UPDATE public.project_task_form_field
SET required = true
WHERE id in
(SELECT f.id
...
--changeset id:update-form-field-participants-sort-order context:weit
UPDATE public.project_task_form_field
...
--changeset id:update-form-field-link-sort-order context:weit
UPDATE public.project_task_form_field
SET sort_order = 8
WHERE id in
(SELECT f.id
...
--changeset id:update-form-field-note-sort-order context:weit
UPDATE public.project_task_form_field
SET sort_order = 9
WHERE id in
(SELECT f.id
...
--changeset id:update-form-field-status-sort-order context:weit
UPDATE public.project_task_form_field
